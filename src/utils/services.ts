  ...
  listenToCustomers(onUpdate: (users: Customer[]) => void): () => void {
    ...
    return onSnapshot(usersCol, (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        ...
        // Ensure roles for known operational accounts
        const email = (mapped.email || '').toLowerCase().trim();
        if (email === 'kurir@laundry' || email === 'kurir@laundry.com') {
          mapped.role = 'courier';
        } else if (email === 'admin@laundry' || email === 'admin@laundry.com') {
          mapped.role = 'admin';
        }
...
export const OrderService = {
  ...
  mapToFirestore(order: Order, customerId: string): any {
    const effectiveCustomerId = (order.customerId && order.customerId !== 'guest_uid')
      ? order.customerId
      : (customerId && customerId !== 'guest_uid' ? customerId : 'guest_uid');
  ...
  mapFromFirestore(docData: any): Order {
    return {
      ...
      customerId: docData.customerId || docData.customer_id || '',
  ...
  listenToCustomerOrders(
    ...
    if (!customerId && !email && !phone) {
      console.warn('[OrderService] No customer identity provided for listening to orders. Returning empty array.');
      onUpdate([]);
      return () => {};
    }
    ...
    return onSnapshot(ordersCol, (snapshot) => {
      ...
      snapshot.forEach((docSnap) => {
        const mapped = this.mapFromFirestore(docSnap.data());
        const isMatchingCustId = customerId && mapped.customerId === customerId;
        const isMatchingEmail = cleanEmail && mapped.email?.toLowerCase().trim() === cleanEmail;
        const isMatchingPhone = cleanPhone && mapped.whatsapp?.trim() === cleanPhone;

        if (isMatchingCustId || isMatchingEmail || isMatchingPhone) {
          list.push(mapped);
        }
      });
      ...
      for (const cachedOrder of cached) {
        const cEmail = cachedOrder.email?.toLowerCase().trim();
        const cPhone = cachedOrder.whatsapp?.trim();
        if (
          ((cleanEmail && cEmail === cleanEmail) || (cleanPhone && cPhone === cleanPhone)) &&
      ...
      console.log(`[OrderService] Realtime customer orders updated for ${customerId || email}: ${list.length} records`);
      onUpdate(list);
    }, (error) => {
      const errMsg = error instanceof Error ? error.message : String(error);
      const isPermissionError = errMsg.toLowerCase().includes('permission') || (error as any).code === 'permission-denied';
      ...
      if (isPermissionError) {
        ...
        const filtered = cached.filter(o => 
          (cleanEmail && o.email?.toLowerCase().trim() === cleanEmail) || 
          (cleanPhone && o.whatsapp?.trim() === cleanPhone)
        );
        onUpdate(filtered);
        return;
      }
