...
export default function App() {
  ...
  useEffect(() => {
    ...
    const isCourierRoute = currentPath === '/courier' || currentPath === '/courier/dashboard';

    if (isCustRoute) {
      if (customer && customer.role === 'admin') {
        navigate('/admin');
      } else if (customer && customer.role === 'courier') {
        navigate('/courier');
      } else if (customer && customer.role === 'customer') {
        setDashboardOpen(true);
      }
    } else if (isAdminRoute) {
      if (customer && customer.role === 'courier') {
        navigate('/courier');
      } else if (customer && customer.role === 'customer') {
        navigate('/customer');
      }
    } else if (isCourierRoute) {
  ...
  const handleSuccessBooking = async (newOrder?: Order) => {
    if (newOrder) {
      ...
      try {
        const orderToSave = { ...newOrder, customerId: activeCustomerId };
        await OrderService.saveOrder(orderToSave, activeCustomerId);
  ...
  if (authLoading && (isAdminRoute || isCourierRoute || isCustomerRoute)) {
    return (
  ...
  if (isCourierRoute) {
    if (customer && (customer.role === 'courier' || customer.role === 'admin')) {
      return (
        <CourierDashboard
          customer={customer}
          orders={orders}
          onUpdateOrder={async (updatedOrder) => {
            ...
            try {
              let custId = updatedOrder.customerId;
              if (!custId || custId === 'guest_uid') {
              ...
              const finalCustId = custId || 'guest_uid';
              const orderToSave = { ...updatedOrder, customerId: finalCustId };
              await OrderService.saveOrder(orderToSave, finalCustId);
              console.log(`[App] Courier updated order ${updatedOrder.order_id} in Firestore.`);
            } catch (err) {
    ...
    // Guest or non-courier accessing /courier -> Render 404 page
    return (
