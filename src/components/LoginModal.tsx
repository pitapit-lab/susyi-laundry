  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (isLoginMode) {
      ...
      try {
        let result;
        try {
          result = await signInWithEmailAndPassword(auth, normEmail, password);
        } catch (signErr: any) {
          // If kurir@laundry or admin@laundry fails to sign in, auto-create account in Firebase Auth
          if (
            (cleanEmail === 'kurir@laundry' || normEmail === 'kurir@laundry.com' || cleanEmail === 'admin@laundry' || normEmail === 'admin@laundry.com') &&
            (signErr.code === 'auth/user-not-found' || signErr.code === 'auth/invalid-credential')
          ) {
            try {
              const isKurir = cleanEmail.includes('kurir') || normEmail.includes('kurir');
              result = await createUserWithEmailAndPassword(auth, normEmail, password);
              const autoProfile: Customer = {
                uid: result.user.uid,
                name: isKurir ? 'Kurir Susyi Laundry' : 'Admin Susyi Laundry',
                email: isKurir ? 'kurir@laundry' : 'admin@laundry',
                phone: '081234567890',
                address: 'Outlet Susyi Laundry',
                points: 0,
                role: isKurir ? 'courier' : 'admin',
                orders: []
              };
              await saveUserToFirestore(result.user.uid, autoProfile, 'Email');
              onLoginSuccess(autoProfile);
              onClose();
              return;
            } catch (cErr) {
              console.warn('Auto create user failed:', cErr);
            }
          }
          throw signErr;
        }
        ...
        // Auto-provision if special accounts
        if (!profile) {
          if (cleanEmail === 'kurir@laundry' || normEmail === 'kurir@laundry.com') {
            ...
            await saveUserToFirestore(user.uid, profile, 'Email');
          } else if (cleanEmail === 'admin@laundry' || normEmail === 'admin@laundry.com') {
            profile = {
              uid: user.uid,
        ...
        if ((cleanEmail === 'kurir@laundry' || normEmail === 'kurir@laundry.com') && password === 'baksourat999') {
