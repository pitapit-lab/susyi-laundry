...
export default function AdminDashboard({ 
  ...
  const [firestoreCustomers, setFirestoreCustomers] = useState<Customer[]>(() => 
    getRegisteredUsers().filter(u => {
      const cleanEmail = (u.email || '').toLowerCase().trim();
      if (cleanEmail === 'kurir@laundry' || cleanEmail === 'kurir@laundry.com' || cleanEmail === 'admin@laundry' || cleanEmail === 'admin@laundry.com') return false;
      return u.role === 'customer' || (!u.role && u.role !== 'admin' && u.role !== 'courier');
    })
  );

  useEffect(() => {
    const unsubscribe = listenToUsersInFirestore((users) => {
      // Filter strictly for users with role === 'customer' or default customer (excluding admin & courier)
      const onlyCustomers = users.filter(u => {
        const cleanEmail = (u.email || '').toLowerCase().trim();
        if (cleanEmail === 'kurir@laundry' || cleanEmail === 'kurir@laundry.com' || cleanEmail === 'admin@laundry' || cleanEmail === 'admin@laundry.com') {
          return false;
        }
  ...
  const customerList = useMemo(() => {
    ...
    const nonCustomerEmails = new Set<string>(['kurir@laundry', 'kurir@laundry.com', 'admin@laundry', 'admin@laundry.com']);
    const nonCustomerPhones = new Set<string>();
    
    // Seed map with registered customers with role === 'customer'
    registered.forEach(user => {
      const cleanEmail = (user.email || '').toLowerCase().trim();
      const userRole = user.role || 'customer';

      if (userRole === 'admin' || userRole === 'courier' || cleanEmail === 'kurir@laundry' || cleanEmail === 'kurir@laundry.com' || cleanEmail === 'admin@laundry' || cleanEmail === 'admin@laundry.com') {
        if (cleanEmail) nonCustomerEmails.add(cleanEmail);
        if (user.phone) nonCustomerPhones.add(user.phone.trim());
        return;
      }
      ...
      const key = phone || user.email || user.uid;
      if (!key) return;
    ...
    orders.forEach(order => {
      ...
      const orderPhone = (order.whatsapp || '').trim();

      // Skip orders associated with couriers or admins
      if (nonCustomerEmails.has(orderEmail) || nonCustomerPhones.has(orderPhone) || orderEmail === 'kurir@laundry' || orderEmail === 'kurir@laundry.com' || orderEmail === 'admin@laundry' || orderEmail === 'admin@laundry.com') {
        return;
      }

      const key = orderPhone || orderEmail;
      if (!key) return;
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-3">
                          ...
                          {/* Status Dropdown (interactive without opening detail) */}
                          <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                            <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider hidden sm:inline">Status:</span>
                            <select
                              ...
                              className={`text-xs font-extrabold px-3 py-1.5 rounded-xl uppercase border cursor-pointer focus:outline-none transition-colors shadow-2xs ${
                      ...
                      <AnimatePresence initial={false}>
                        {isExpanded && (
                          <motion.div
                            ...
                            className="border-t border-slate-150 p-6 bg-slate-50/40 space-y-6"
                          >
