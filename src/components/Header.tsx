          ...
          {customer ? (
            ...
            >
              <div className="w-6 h-6 rounded-full bg-purple-600 text-white font-serif text-[10px] font-bold flex items-center justify-center">
                {customer.role === 'admin' ? '🛡️' : customer.role === 'courier' ? '🚚' : ((customer.name || '').substring(0, 2).replace('K', '').replace('a', '').trim() || 'C')}
              </div>
              <span className="max-w-[130px] truncate">
                {customer.role === 'admin' ? 'dasboard admin' : customer.role === 'courier' ? 'Portal Kurir' : (customer.name || '').replace('Kak ', '')}
              </span>
            </button>
          ) : (
            /* Login Entry Button */
